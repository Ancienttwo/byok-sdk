...
--   deploy/sql/0010_tenant_erasure.sql (package/operator erasure receipt, not product data)
--
...
BEGIN
  ...
  IF port_tables < 27 THEN
    RAISE EXCEPTION
      'control-plane invariants ran against an unmigrated schema: %.% has % port table(s), expected at least 27 (0001_cloud_local.sql + 0002_core_domain.sql + 0003_cloud_cleanup.sql + 0004_device_proof_truth.sql + 0005_skill_packs.sql + 0007_approval_timeline.sql + 0008_device_assertion_replay.sql + 0010_tenant_erasure.sql; 0009_task_cancellation.sql is checked separately)',
      current_database(), current_schema(), port_tables;
  END IF;
END $$;

-- ---------------------------------------------------------------------------
-- 0.2 Tenant-erasure receipt is installed, tenant-first, and distinct from
...
BEGIN
  ...
  IF missing_columns IS NOT NULL THEN
    RAISE EXCEPTION
      'control-plane invariants ran without 0010_tenant_erasure.sql: tenant_erasure_operation is missing column(s): %',
      missing_columns;
  END IF;
END $$;
