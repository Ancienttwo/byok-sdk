...
-- wrong". 18 is 0001's seven plus 0002's eleven; a later migration may only
-- raise it.
DO $$
DECLARE
  port_tables integer;
BEGIN
  ...
  IF port_tables < 18 THEN
    RAISE EXCEPTION
      'control-plane invariants ran against an unmigrated schema: %.% has % port table(s), expected at least 18 (0001_cloud_local.sql + 0002_core_domain.sql)',
