...
## P3d supervisor scope (after89e323b4 terminal partial PASS)

P3d combines the earlier P3e SDK inline-extension startup closure with remaining package discovery: pi-subagents async jiti lookup and rpiv-todo i18n imports must become build-resolved/static without createRequire, try-import fallbacks, scanner changes or feature removal; trusted-launch-cwd/team-pi-session/team-pi-relay/create-daemon paths must use release authority or be demonstrably development-only. create-daemon.ts is newly registered for its constructor native identity lookup, not wider daemon changes. Build/plugin/package source registration will be narrowed after read-only dependency trace if needed; no node_modules mutation as a production solution.

Pi RPC CLI argument failures use the existing failUsage renderer including the expected-argument recomputation; pure parser tests remain possible and runtime/model error semantics are not silently changed. Acceptance: S2 tripwire only native clipboard12requests (still RED on unchanged zero-attempt assertion), all other required gates green; actual two-lane get_state and unchanged containment assertions, no attribution trailers. Native1006 body and publication remain outside scope. Product and registration commits separate; one frozen final gate; no push/merge/publish.

## P3d helper-lookup activation (supervisor accepted)

- Configured preparation initialization resolves attested pi-prepared identity through SDK authority and static pin/provenance. One instance Promise is awaited by start before control exposure; refusal fails start. Public createDaemon remains synchronous. Only unconfigured authority selects installed dev discovery.
