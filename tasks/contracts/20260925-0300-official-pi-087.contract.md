...
本轮完成已批准计划 M1/M2/M3：官方 0.87.1 依赖与 breaking changes 适配，公开 API prepared lane、A1″/A2′、最终字节门、两层 retry 关闭、SDK RPC frame bound 与空工具集，以及官方 closure 身份与打包。只本地 commit，不 push 或调用 provider。

## Scope

- In scope: M1/M2/M3 所需 packages、依赖声明和 lock、对应测试、spec 与任务工件。
- Out of scope: M4 递归验收、M5 provider 探针、发布、Salesko、fork 修改。
