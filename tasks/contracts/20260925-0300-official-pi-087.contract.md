...
已完成 M1/M2/M3 并通过独立 gate；本轮按 w2:pB 派工执行 M4：五条递归边、custody、workflow/pi-subagents 回归，以及续轮 typed refusal 传递。此前范围：官方 0.87.1 依赖与 breaking changes 适配，公开 API prepared lane、A1″/A2′、最终字节门、两层 retry 关闭、SDK RPC frame bound 与空工具集，以及官方 closure 身份与打包。只本地 commit，不 push 或调用 provider。

## Scope

- In scope: M4 五边/custody/workflow/官方嵌套安装回归、续轮 typed refusal 及对应 packages/tests、spec 与任务工件。M1–M3 已验收。
- Out of scope: M5 provider 探针、发布、Salesko、fork 修改。
