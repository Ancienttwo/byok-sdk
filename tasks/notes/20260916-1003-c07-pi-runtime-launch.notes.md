...
## P3d decision boundary and usage-only delivery

Product21ab3c86: RPC argument parser shares one rejection callback for digest and all other flags; host injects failUsage, including policy projection reparse and interpreter/loader usage rejection. Pure parsers still throw; runtime/model semantics unchanged. Client build/typecheck and23targeted tests pass; logs `_ops/c07-identity-workspace/p3d/usage-{build,typecheck,tests}.log`. No full/pack repeated before combined freeze. Supervisor read-only acceptance of this bounded diff received; not overall P3d gate. Attribution0.

Read-only extension/path traces show the requested closure crosses a real new authority boundary. async-execution.ts535-564 launches PATH node+jiti+adjacent TSrunner; runner is absent from client dist and later launches Pi/loads computed exportHTML. Current assets explicitly exclude code; parent identity.launchArgv cannot be retargeted to runner. Supervisor prefers daemon-resolved self+descendant bindings covered by complete config checksum, with explicit multi-kind record support and Salesko P4 authoring in one change. **Runner implementation paused by supervisor pending that record shape**; no extension edits, dependency changes, scanner exceptions or reduced surface.

Decision Packet complete in handoff§96, local identical `_ops/c07-identity-workspace/p3d/decision-packet.md`: A local maintained integration/precompiled same-bundle runner; B upstream/published fork; C Owner-approved product loss. Each records provenance/maintenance/product/distribution impact and10x/negative controls. Three binding routes(a)daemon predeclared runner/(b)generic dispatch prefix/(c)bounded daemon-issued descendant rules are compared; (a)recommended with mandatory record/P4 effect. SDK inline factories and capability-ceiling imports must share one source.
