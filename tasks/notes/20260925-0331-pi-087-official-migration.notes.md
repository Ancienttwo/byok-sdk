...
## WP2b — test port to official 0.87.1

Subject: worktree `claude/pi-087-migration` on 9c54ff1f (WP4), uncommitted. Runner is `npx vitest run` from `packages/client`; `bun test` does not run this suite (`vi.waitFor` and similar are missing, so it reports false failures).

### Per-file port

- `input-preparation-message-support-set.test.ts`: the native-projection block now drives `buildPreparedTranscriptMessages`. Host-canonical text becomes an official `AssistantMessage` with the A2' sentinel provenance, zero usage and `stopReason: "stop"`, and `origin` does not leak. The case "nothing fabricated (no api/provider/model/usage)" was replaced: on the official kernel the sentinel IS the provenance, by design (A2'). The type-level check now targets pi-ai `AssistantMessage`/`UserMessage`. Added builder refusals (`prepared_transcript_invalid`) for no-origin assistant, provider origin, toolResult and multimodal user. The builder does not re-refuse the parse-layer cases (extra keys, array content). Those are still covered by the control-protocol and schema describe blocks.
- `input-preparation-model-parity.test.ts`: uses `verifyPreparedPiInput` + `PreparedSessionError`/`canonicalPreparedValue` (prepared-request). The compile snapshot uses `customPrompt`. D asserts: the system message is exactly customPrompt, there is no `strict`, and no sentinel appears. The "session model the fork composes from models.json" case became "the session model the official runtime registers": real `ModelRuntime.create({modelsPath:null})` + `registerPreparedProvider`, compared on the drift-check fields, with the provider-id control kept. **todo**: whitespace-only provider id. See gap G1.
- `mcp-projection.test.ts`: import only.
- `pi-prepared-launcher.test.ts` (32 cases, all green against built dist): the digest helper uses the SDK `canonicalPreparedValue`, and the snapshot carries `customPrompt`. The reservation case is split per deviation 2. (a) A concurrent `prompt` is refused (`Agent is already processing`) and the provider sees only D. (b) A `steer` is accepted. The session then issues request 2, the gate refuses it, the provider sees only D, and `get_messages` shows the steer user message followed by an assistant `stopReason: "error"` ("Connection error."). The typed `prepared_context_drift` lives only in run-scoped gate state and cannot be read over RPC. The tamper case is split per deviation 6: a bare edit gives `prepared_digest_mismatch`, and a re-digested, rebound edit gives `prepared_context_drift`, both with zero provider requests.
- `pi-input-preparation.test.ts`: identity checks the official tuple (no `byokFork`, `v0.87.1`, gitHead `f07218c4…`, forkBuild 0, lockstep pi-ai/pi-agent-core at the same version, new format tags). Compile asserts the system message == customPrompt and no `strict`. The closure walk now starts at the two modules `loadOfficialCompiler` imports. Result: third-party is exactly `openai, partial-json, typebox` (the fork also had `cross-spawn, fs, ignore, os, path, url, yaml`), builtins exactly `node:fs` (via `utils/provider-env.js` `require("node:fs")`, Bun-compiled-binary only), and a 39-file pi-ai set pinned. `openai-completions.js` names no forbidden builtin or `process.env`. Envelope-contract cases use the new tags and v1 shape. Added refusal cases for a renderer input and for a missing customPrompt. **todo**: `{type:"object"}` without properties. See gap G2.
- `prepared-launch-response-timing.test.ts`, `pi-runtime-launch-cwd.test.ts`: `customPrompt` added. The cwd file now asserts that D's system message == customPrompt, with no `<cwd>` and no Agent-home path. The process-cwd assertions are unchanged.
- `runtime-launch-description.test.ts`: official provenance values and compiler-4 format tags.
- `pi-runtime-host-binding.test.ts`: the manifest fixture has no `byokFork`, and the refusal text is `Pi nativeProvenance.${field} differs from the official runtime identity`.
- `pi-s2-bundle-resolution.test.ts`: the fixture has `customPrompt`. The official prepared host answers the first stdin frame itself and admits only `prompt_prepared`, so the `get_state` startup probe on `pi-prepared` now gets its typed refusal (`prepared_input_invalid`, "the first frame of a prepared host must be prompt_prepared"). That refusal is taken as startup evidence: it is written only after the static graph loads, config/binding verify and the provider registers. `pi-rpc` keeps the `get_state` check. Containment, escape, registry and zero-provider-request assertions are unchanged. 2 passed, 0 skipped (darwin).
- `fixtures/prepared-compile-snapshot.ts`: official shape (`customPrompt` = `PREPARED_COMPILE_SYSTEM_PROMPT`, empty renderer inputs).

### Production gaps found (tests marked `it.todo`, production untouched)

- G1: `adapters/pi/input-preparation.ts:425` `validatePreparedModel` must refuse `model.provider.trim().length === 0` (InputPreparationCompileError). The fork refused it and the official compile accepts it.
- G2: `adapters/pi/prepared-request.ts:220-221` `projectTool` must refuse a schema whose `properties` is missing or not an object. The fork refused it (`pi-wt-086 core/input-preparation.ts:280-287`, "expected full object tool schema"), and `{type:"object"}` now compiles.

### Purity probe (`fixtures/pi-compile-purity-probe.mjs`, `pi-compile-purity.test.ts`)

- Load subject: the client dist bundles `compilePreparedPiInput` but does not export it, so it cannot be loaded from `dist/`. The parent test bundles `{ compilePreparedPiInput, loadOfficialCompiler }` from source in `beforeAll` with tsup's esbuild, inlining SDK/workspace code and leaving third-party imports bare, into `node_modules/.cache/byok-pi-compile-purity-*/compile-entry.mjs`. Bare imports therefore resolve to the installed pi-ai, and the "load reads only inside installed packages" rule holds unchanged. The probe imports it after monitors are installed, and `loadOfficialCompiler()` runs in the load phase, as the production compiler does before compiling. Attribution: `official:` for the pi-ai roots (link + realpath), `sdk:` for the bundle. Monitoring code is unchanged. The negative-control anchor moved to pi-ai `dist/utils/text.js` `getSystemMessageText` (the serializer renders D's system message with it). All five controls fire and are attributed there.
- D from the probe is byte-equal to the in-process compile of the shared fixture. D is byte-identical under both forced skews and both poisoned homes.
- Import time is narrower than 0.86: zero non-runtime load events and zero non-runtime load env reads (the fork had `config.js` package.json reads, `PI_PACKAGE_DIR`, `os.homedir`, and `OSTYPE` from cross-spawn). No home, agent or credential path is touched in any phase.
- Compile time, new accesses (the fork compile never entered `stream`):
  - **Env reads, every compile (cold and warm)**: `OPENAI_ADMIN_KEY`, `OPENAI_ORG_ID`, `OPENAI_PROJECT_ID`, `OPENAI_WEBHOOK_SECRET`, `OPENAI_LOG`, `OPENAI_CUSTOM_HEADERS` at `openai/internal/utils/env.mjs:11`. These are openai 6.40.0 client-constructor defaults (`client.mjs:70,153,159`), because A1' builds a real client inside `streamSimple`. None reaches D's body. On the live path, though, the same client turns `OPENAI_ORG_ID`/`OPENAI_PROJECT_ID`/`OPENAI_CUSTOM_HEADERS` into request headers to the BYOK endpoint, and `OPENAI_LOG` into logging. The env-read gate was not relaxed. 6 cases fail on this one finding: "touches no … ambient surface", "reads no environment variable", "ambient independence" (its purity part), the clock and getRandomValues controls (their `purityViolations(...)==[]` part), and "proves purity … for the clean run". Allowlisting, or scrubbing the host env, is an owner decision.
  - Clock/generator: `Date.now` at pi-ai `api/openai-completions.js:170` (the output message's in-memory timestamp). `Math.random` at `openai/client.mjs:344` (log correlation id) and `Date.now` at `:346`/`:360` (timing). The forced-skew case proves none reaches D, so they were added to `ALLOWED_COMPILE_NONDETERMINISM` with that justification. That is the file's documented procedure for this situation, not a relaxation.
  - No fs, process, network or worker event in either compile phase.
