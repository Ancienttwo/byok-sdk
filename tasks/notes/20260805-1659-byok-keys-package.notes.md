...
- 2026-08-05 K1 complete: SecretStore layer landed as six source modules (`secret-name`, `command-runner`, `secret-store`, `macos-keychain`, `windows-credential-manager`, `secret-scope`) plus `errors.ts` growing from 12 to 29 codes. 158 new tests, package total 259, monorepo total 1488. Both OS backends are tested only through an injected `CommandRunner`; no test reads or writes a real credential store.
