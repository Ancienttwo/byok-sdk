...
> **Status**: Active
> **Plan**: plans/archive/plan-20260809-0001-s4b-c-cloud-cleanup.md
> **Contract**: tasks/archive/contract-20260915-0438-s4b-c-cloud-cleanup.md
> **Review**: tasks/archive/review-20260915-0438-s4b-c-cloud-cleanup.md
...
- mailbox retention 从同一 `deleted RETURNING` CTE 计算释放量，避免 concurrent ACK 夹在预查与 DELETE 之间造成计量漂移。
- reservation admission 与 tombstone candidate 都锁同一 tenant entitlement row；`delete_pending` 拒绝新 object reservation，消除 mark 后重新 reservation 的删除竞态。
- delete cursor 是轮转公平游标：满页越过 poison 让后续 hash 前进，空/短页清 cursor 后重新尝试失败 tombstone。
- replay row 记录 `replay_source_seq`；device-scoped operator id 绑定 exact dead-letter source，而非只绑定等值 bytes。
...
- 独立 Claude review 找出并已修复四个真实缺陷：retention 双扫描计量竞态、tombstone 后 reservation 竞态、delete poison-pill 饥饿、跨 source replay-id TOCTOU。cursor 永久跳过、R2 entity/invalid-key、tenant-global replay id 等报告经完整契约复核为非缺陷；最终 reservation/cursor 与 replay 两个 post-fix 包均返回 `No findings`。
