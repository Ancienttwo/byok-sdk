...
- Current hardening uses the same closed 1 MiB read authority for daemon/doctor and refuses oversized fix；serializes pair/renew writers；requires bounded no-follow owner records with process-start identity and no bare-PID fallback；pins the verified quarantine directory inode during synchronous publication；and reads the unpair identity only after cleanup lease acquisition. New exact-SHA acceptance is still required；Claude is paused and was not invoked.
