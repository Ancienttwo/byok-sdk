...
- Current hardening adds a pathname-derived kernel-exclusive transition mutex、pre/post-open/read device/audit binding、POSIX quarantine-directory-before-unlink and source-parent-after-unlink fsync ordering、all-platform falsifiers，and the complete diagnostics/lifecycle Windows suite. New exact-SHA acceptance is still required；Claude is paused and was not invoked.
