...
- Current hardening binds every sensitive Windows pathname to the opened regular-file identity before reading；publishes evidence/manifest before source removal with rollback；caps quarantine scan/read work at 200 entries/128 MiB；repairs the remove-under-lease assertion and adds pair/unpair coverage；and adds the diagnostics/lifecycle suite to Windows CI. New exact-SHA acceptance is still required；Claude is paused and was not invoked.
