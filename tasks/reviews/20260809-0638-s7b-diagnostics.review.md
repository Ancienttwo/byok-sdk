...
- Current hardening serializes every daemon lifecycle mutation；pins and validates bounded no-follow quarantine manifests/evidence；uses a kernel-owned liveness listener to distinguish the original owner from PID reuse without external commands；and corrects the runbook. New exact-SHA acceptance is still required；Claude is paused and was not invoked.
