...
import { tenantId as sdkTenantId } from '../../packages/core/dist/index.js';
import { createEnvelope } from '../../packages/protocol/dist/index.js';
import { createHarness } from '../../packages/cloud/src/__tests__/support/harness';
...
for (const cancelFirst of [false, true]) test(`Salesko transaction + SDK recurring replay: cancelFirst=${cancelFirst}`, async () => {
  ...
  const harness = createHarness({ agentMessage: { consume: async ({ tenant: actualTenant, deviceId, taskId, context, payload }) => {
  ...
  const device = await harness.pairDevice(tenant);
  ...
  await harness.cloud.submitRecurringExecution(tenant, { taskId, deviceId: device.deviceId, payload: {
  ...
  const publish = () => harness.request('/byok/messages', { method: 'POST', headers: { ...device.authorization, 'content-type': 'application/json' },
