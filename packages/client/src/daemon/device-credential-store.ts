...
public static class ByokDeviceCredential {
 public sealed class ReadResult { public int ErrorCode; public byte[] Value; public int DiagnosticStage; public int DiagnosticKind; public int DiagnosticHResult; }
 ...
 private static ReadResult Failure(int stage,Exception e) { return new ReadResult { ErrorCode=0,Value=null,DiagnosticStage=stage,DiagnosticKind=Kind(e),DiagnosticHResult=e.HResult }; }
 public static ReadResult Get(string t) { IntPtr p=IntPtr.Zero; bool found; try { found=R(t,1,0,out p); } catch(Exception e) { return Failure(1,e); } if(!found) return new ReadResult { ErrorCode=Marshal.GetLastWin32Error(),Value=null }; ReadResult result; try { C c=(C)Marshal.PtrToStructure(p,typeof(C)); byte[] b=new byte[c.CredentialBlobSize]; if(b.Length>0) Marshal.Copy(c.CredentialBlob,b,0,b.Length); result=new ReadResult { ErrorCode=0,Value=b }; } catch(Exception e) { result=Failure(2,e); } try { CredFree(p); } catch(Exception e) { return Failure(3,e); } return result; }
 public static int Delete(string t) { if(D(t,1,0)) return 0; return Marshal.GetLastWin32Error(); }
}
"@
$stage=1; try { $r=([Console]::In.ReadToEnd()|ConvertFrom-Json); if($r.operation -eq 'replace'){$stage=2;$secret=[Convert]::FromBase64String([string]$r.secret_base64);$stage=3;$code=[ByokDeviceCredential]::Set([string]$r.target,[string]$r.username,$secret);if($code -ne 0){[Console]::Error.Write(('credential operation failed (win32={0})' -f $code));exit 1};exit 0}; if($r.operation -eq 'read'){$stage=4;$result=[ByokDeviceCredential]::Get([string]$r.target);if($result.DiagnosticKind -ne 0){[Console]::Error.Write(('credential operation failed (stage={0},kind={1},hresult={2})' -f $result.DiagnosticStage,$result.DiagnosticKind,$result.DiagnosticHResult));exit 1};$code=$result.ErrorCode;if($code -eq 1168){exit 44};if($code -ne 0){[Console]::Error.Write(('credential operation failed (win32={0})' -f $code));exit 1};$stage=5;[Console]::Out.Write([Text.Encoding]::UTF8.GetString($result.Value));exit 0}; if($r.operation -eq 'clear'){$stage=6;$code=[ByokDeviceCredential]::Delete([string]$r.target);if($code -eq 1168){exit 44};if($code -ne 0){[Console]::Error.Write(('credential operation failed (win32={0})' -f $code));exit 1};exit 0};exit 2 } catch { $root=$_.Exception; $e=$root; $code=$null; while($null -ne $e){if($e -is [System.ComponentModel.Win32Exception]){$code=$e.NativeErrorCode;break};$e=$e.InnerException}; if($null -ne $code){[Console]::Error.Write(('credential operation failed (win32={0})' -f $code))}elseif($null -ne $root){$kind=9;if($root -is [System.Management.Automation.MethodException]){$kind=2}elseif($root -is [System.Management.Automation.MethodInvocationException]){$kind=3}elseif($root -is [System.SystemException]){$kind=4}elseif($root -is [System.Management.Automation.RuntimeException]){$kind=5};[Console]::Error.Write(('credential operation failed (stage={0},kind={1},hresult={2})' -f $stage,$kind,$root.HResult))}else{[Console]::Error.Write('credential operation failed')}; exit 1 }
