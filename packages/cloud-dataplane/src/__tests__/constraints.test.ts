...
describe('the CI dataplane job', () => {
  ...
  it('runs BOTH compositions of the cloud conformance suite', () => {
    ...
    expect(workflow).toContain('bun run --filter @byok-sdk/cloud-dataplane test');
...
describe('the CI bun toolchain', () => {
  ...
  it('pairs every Bun setup with one frozen install', () => {
    const setups = workflow.match(/oven-sh\/setup-bun@v2/g) ?? [];
    const installs = workflow.match(/run: bun ci/g) ?? [];
    expect(setups.length).toBeGreaterThan(0);
    expect(installs).toHaveLength(setups.length);
  });
});
