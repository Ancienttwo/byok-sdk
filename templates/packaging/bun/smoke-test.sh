...
#
# Proves the M3-K packageability guarantee for the bun-compile path: builds
# examples/packaging/launcher.ts into a single bun-compiled executable (via
...
WORK_DIR="$(mktemp -d "${TMPDIR:-/tmp}/byok-bun-smoke.XXXXXX")"
...
if [ "$OS" = "Windows" ]; then
  BIN="$WORK_DIR/build/launcher-bun.exe"
else
  BIN="$WORK_DIR/build/launcher-bun"
fi
...
assert_probe() {
  ...
  if [ "$exit_code" -ne 0 ] || [ -z "$line" ]; then
    ...
    echo "CRITICAL: pi resolution did not degrade catchably under bun --compile. Full output:"
    cat "$out"
    exit 1
  fi
...
EXIT2=0
( cd "$ISOLATED_DIR" && BYOK_PI_BIN="$STUB" "./$(basename "$RUN_BIN")" ) >"$OUT2" 2>&1 || EXIT2=$?
assert_probe "pi-stub-picked-up" "true" "$OUT2" "$EXIT2"

echo "==> bun packageability smoke: PASS"
