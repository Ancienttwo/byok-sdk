...
- [ ] K4 Swap back into aip-main-open: **registry publish complete** (`@byok-sdk/keys@0.1.0`, `latest` read back on 2026-08-08); consumer swap remains in draft PR #7. Diff baseline `c6a5385..HEAD` for drift, delete the ported code, switch to the npm dependency, convert the two `instanceof LocalExecutionError` sites (`settings.ts:358`, `providers.ts:1673-1677`) to structured code detection, and require `apps/local-agent/src/settings.test.ts` (`:313-318`) to pass unchanged
