...
- [ ] P2b Deliver the minimal SDK-owned ordinary in-process entry with explicit session cwd and all five inline factories, plus callable prepared entry/helper dispatch. This prerequisite moves forward from P3 so P2 never claims a cwd-only fix with an unusable entry. Keep bundle containment/loader closure acceptance in P3.
