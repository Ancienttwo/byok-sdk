...
- [ ] **新 subject 重验**：focused guard、client/root build/typecheck/test 与 strict workflow 已通过；product branch 的 strict architecture gate 仍要等独立 reconciliation branch 集成后才能清零并生成 fresh checks。旧 `external_pass` 因 production source 变化转 stale；push、remote CI、fresh external review 与 merge 继续需要独立授权。
