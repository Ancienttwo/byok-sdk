...
U3 must not add another `clientVersion` authoring path. After Slice A code
freeze, U4 adds one release-hygiene slice under an amended contract rather
than creating a competing release plan.

### U4b scope

- prove the packed `@byok-sdk/keys` artifact depends on the intended published
  `@byok-sdk/core` line without downstream overrides; repair the currently
  published keys/core `0.4.2` metadata skew in the next independently versioned
  keys artifact;
- keep every package engine at Node.js `>=22.22.0`;
- require aligned-version policy, changelog, package graph, clean-candidate pack
  smoke, and registry/dist-tag readback as one release gate;
...
- distinguish local pack evidence from registry evidence: neither authorizes
  publish, and a release is not complete until an explicitly authorized publish
  is followed by exact registry/tarball readback.

### U4b acceptance

- packed keys metadata installs against its declared packed core dependency
  with no workspace override;
- package graph and engine floors are exact and changelog covers public API and
  forward migrations;
- clean-SHA pack/smoke proves CLI/manifest identity and packed dependency graph;
- when publish is separately authorized, registry version, dist-tag, tarball,
  dependency metadata, migration checksum, and downstream exact-pin smoke are
  read back; without that authorization the plan stops at local evidence.
...
- U4b owns package manifests, release scripts, changelog, and pack evidence;
  it does not edit U1/U2/U3/U5 implementation surfaces concurrently.
- The existing contract must be amended before U4b source edits; this plan
  update alone grants no new allowed paths or external mutation authority.

### U4b tasks

- [ ] Audit live registry and packed keys/core metadata before choosing versions.
- [ ] Amend the U4 contract with exact manifest/release-script/changelog ownership.
- [ ] Add a failing packed-dependency regression and repair keys/core metadata.
- [ ] Run package graph, full required checks, and one clean-candidate pack smoke after code freeze.
- [ ] Record local release evidence; stop before publish unless separately authorized.
