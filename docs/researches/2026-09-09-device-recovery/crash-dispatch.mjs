...
if(mode){
 ...
 let byok,http,daemon;let startupEvidence;
 try {
  ...
  const adapter={descriptor:freezeRuntimeAdapterDescriptor({id:'pi',supportsDispatchSelection:true,requiresMcpToolsetToolObservation:false,capabilities:{steer:false,resume:false,approvalInteractive:false,mcpToolsets:false,permissionModes:['auto']},environmentRequirements:{credentialNames:[]}}),async detect(){return {kind:'available',version:'fixture-1'};},async prepare(){return {kind:'prepared',operation:{async start(input){
   ...
   const fd=await fs.open(path.join(root,'execution-audit.jsonl'),'a',0o600);try{await fd.write(JSON.stringify({instruction:input.instruction})+'\n');await fd.sync();}finally{await fd.close();}
  ...
  if(mode==='cleanup'){await daemon.unpair();process.send?.({type:'cleaned'});}
   ...
   if(mode==='first'){
    ...
    process.send?.({type:'recovered',beforeIds:before.tasks.map(t=>t.taskId),afterIds:after.tasks.map(t=>t.taskId),replayTaskId:h.taskId,executions:audit.length,sameInstruction:audit.every(a=>a.instruction===cfg.request),hostMappingExists:false});
 ...
 try {
  ...
  await fs.writeFile(path.join(import.meta.dirname,'crash-report.json'),JSON.stringify(report,null,2)+'\n');console.log(JSON.stringify(report,null,2));
 }
}
