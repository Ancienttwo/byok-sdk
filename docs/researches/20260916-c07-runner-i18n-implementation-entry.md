...
| ⑪ (M2b-1) | Publish ./export-html exporting exportFromFile through the native package boundary, with PI_PACKAGE_DIR/assetRoot resource resolution and explicit output path-policy. Required before M2b-3 static share import;1005 has no such export |
...
M2b-0 implementation7cabc200 freezes the above reason mapping without changing M0 fixtures. Finite policy/parent limit declaration checks retain their own frozen reasons. For otherwise admissible limits and verified edge/parent, charged transition plus actual parent.depth >= effectiveLimits.maxDepth selects exhausted first (including0); when budget remains, candidate depth beyond caps selects exceeded. Zero-charge runner→print is exempt from exhaustion at the cap. Thus the same over-cap candidate cannot select both depth refusals. Remaining-depth arithmetic and env projection mismatch have separate named refusals. Supplied DEPTH/MAX_DEPTH values use exact String() equality; missing/null remains absent. Twenty original vectors and10 additional partition/projection tests pass through real validators; recursive enablement still awaits five custody execution proofs and loader prerequisites.
...
## M2b-3 enablement design draft — no product authorization

**P1 / frozen base.** M2a and M2b-0 implement strict plan/composition checks; M2b-1 is accepted for assets/preverify only. `sdk-reserved-helper-host.ts:91–92` still explicitly rejects runner/print. `runtime-descendant-plan.ts:49–58,85–97` proves finite kind completeness and common identity, not live-instance custody. `descendant-launch.ts:134–167` validates independently supplied parent transition/depth/env; it cannot manufacture that parent state. Existing upstream references below are under `packages/client/node_modules/pi-subagents/src` at0.60.0; they are insertion candidates for the later single vendored integration, not registered product paths or claims of completed enforcement.

**P2 / proposed execution order.** Verified daemon plan → current verified parent instance and root budget → validate requested edge/user limits → atomically reserve fanout/session admission and acquire session/parallel exclusion → construct typed perLaunch from that parent (never env) → write one config byte sequence/digest → construct final child env from the declared projection and authorized credential inheritance → assertDescendantSpawn against independent expected parent immediately before spawn → child verifies its config/invocation/physical binding before native entry → terminal/cancel joins child and releases only concurrency resources. Uncommitted admission failure rolls back only its own reservation. Already consumed authorization is not restored by cleanup. Exact charge commit points and crash recovery remain design obligations below; this ordering is not an implementation claim.

| Custody obligation | Real current execution seams / missing enforcement | Proposed test at the real seam; stop criterion |
| --- | --- | --- |
| 1. Atomic fanout admission | runs/shared/run-fanout-budget.ts:228–267 wraps claim writes and callback with admission lock/rollback; dynamic background/subagent-runner.ts:3925–3949 and foreground/subagent-executor.ts:6310–6314 consume it. Current lock is evidence of a mechanism, not complete daemon-root binding. | Barrier-start competing real process admissions against one root/finite cap; accepted claims never exceed cap and actual starts match committed admissions. Inject failure before callback commit and assert only this transaction's claims removed; pre-existing claims unchanged, capacity reusable exactly once. Post-spawn failure must not refund a consumed authorization. Partial spawn/commit and crash ambiguity require a proved state transition before enablement, not a catch-to-refund. |
| 2. Session cap and parallel/session-file ownership | runs/shared/parallel-utils.ts:187–235 joins workers/releases semaphore; runs/shared/spawn-budget.ts:10–80 has mutable session counts and nullable unlimited mode; async-execution.ts:1021–1044 accepts shared sessionFile; runner:5539–5603 revival lease is not general exclusion. | Explicit finite sessionCap checked against immutable policy; undefined/unlimited and requested bounds above policy reject. Two attempts for the same validated session file must not overlap, including distinct runner instances. Different sessions obey global declared parallel cap. Record PID/start/terminal and maximal active count; cancel/failed spawn releases each acquired concurrent slot once after target termination. Session identity/ownership persistence and canonical file key need a concrete author before enabling; process-local semaphore alone is insufficient across descendants. |
| 3. Charge once runner→print | runner runPiStreaming:1805–1829 and actual spawn:608–648; foreground execution.ts:558–588 is a separate direct print edge. Shared validator already recognizes runner→print as zero charge and checks parent path. | Drive each of five frozen edges through real spawn. Charged edges increment depth exactly once; runner→print preserves depth/instance at cap. Model retry changes attempt/model only, never manufactures a new root, depth reset or quota. Mutation: pass child-authored expectation or reset within cap; require zero starts and exact refusal. Exhausted/exceeded fixtures stay byte-identical. |
| 4. Consumable permit vs concurrency slot | shared/workflow-child-permit.ts:59–110 available→claimed→consumed, wrong child permanently consumes; foreground execution.ts:568 consumes before spawn. createWorkflowChildPermit has no known production issuer; scripted-workflow semaphore is a separate concurrency mechanism. | If this path is supported, prove a real issuer and single allowed consumption; duplicate/retry/mismatched child refuses, cancellation never turns consumed back to available. Separately test slot release on terminal/cancel. No stub permit or newly invented issuer from child config. Scripted-workflow Owner disposition determines reachable scope; an excluded path needs built-graph and request-refusal proof, not deletion of the obligation by assertion. |
| 5. Actual parent/root budget custody | runs/shared/pi-args.ts:858–930 and shared/types.ts:2693–2718 currently derive depth/parent from env. Plan templates and config digests do not establish a live parent instance or make a replayed sibling authorized. | Expected parent is selected from verified parent state, with daemon-origin rootTaskId and root budget; perLaunch fields are projections covered by config digest. Compare PI_SUBAGENT env only after typed transition. Test sibling replay, stale parent, substituted root namespace, same-root duplicate claim, in-cap depth reset, and concurrent cancel/retry. All refuse before spawn; a self-consistent forged child context must not establish expected state. Concrete persistent root/instance ownership and admission transaction are still MISSING; stop until supplied and traced. |

The plan chain supplies immutable executable permission; it is not an expanded tree of signed runtime instances. Config digest is neither a signature nor OS isolation. Keep the existing same-UID limitation honest. Credentials inherit only on declared edges; tests use fake values, verify no values enter config/digest/log, and prove MCP grandchildren receive projectPiMcpEnvironment rather than raw Pi env. No new custody mechanism or Host policy defaults are authorized by this draft.

### Helper enablement order and closure gates

1. **Entry gates first:** M2b-1 accepted asset prerequisite; native1006 item11 actual public export subpath/resource behavior/output path-policy; Owner M2b-2 α/β/γ decision implemented and verified; scripted-workflow disposition and five custody execution proofs. P4 Host author must supply the approved exact records/policy. Current blockers remain visible; no native implementation/publish permission inferred.
2. **Terminal print implementation first, unreachable from production dispatch:** same-bundle `pi-subagent-print` uses native runPrintMode rather than RPC/CLI discovery. Verify its own full config/binding, initial model/task/session/env, no user-selected interpreter/PATH. Tests may directly exercise the future internal handler only after its product paths are registered. Public runner/print dispatch refusal stays until the whole enabling slice passes.
3. **Runner implementation second, still unreachable from production dispatch:** replace async Node+jiti launch and runner→Pi discovery with the exact interpreter/artifact/prefix from verified templates. Runner→print uses the zero-charge transition, immutable policy and real root budget. Static export/structured-output graph must match the selected feature contract. No config/record retargeting and no private native import.
4. **All edges integrate before activation:** rpc→runner, rpc→print, runner→print, print→runner, print→print. Prepared has no invented descendant edge. One integration cutover retires old authorities; do not ship a runner-enabled/print-disabled intermediate or a depth-based silent feature fallback. Enable helper dispatch only in the sole M2b-3 product slice after frozen full/pack gate plus real recursive/cancel acceptance.
5. **Acceptance does not mean full C07:** S1 installed Node and S2 real sealed artifact execute the same approved surface; jiti tripwire12 must disappear without guard edits. Clipboard remains a separate1006 closure condition until verified. External publication/merge/install still require their own approval.

### Downstream notice — proposed wording

“Attested runtime admission requires a complete Host-authored kind plan. If descendantPolicy.maxDepth > 0 and the selected self kind has reachable edges, every reachable kind must have its dedicated record with the same release tuple, revision, policy and controlled roots. Missing runner/print rows refuse admission; the SDK does not reduce the policy to self-only or discover an installed executable. maxDepth=0 is an explicit Host policy disabling charged delegation, not a repair the SDK applies automatically. Prepared with no outgoing edges may use its valid self-only plan. Record completeness does not enable SDK runner/print helpers before the custody implementation is accepted.”
