...
### 19. G3 executor implementation identity — SDK half landed, preparation half blocked — 2026-09-15

**Landed.** `packages/client/src/daemon/tool-implementation-identity.ts` is the single authority for what an implementation identity is: the `ToolImplementationIdentityV1` union, the seven unavailable reasons, the `ToolImplementationAuthority` interface the SDK declares and never implements, the measurement that turns one host install record into an attested identity, and the reverification every spawn runs. `adapters/pi/input-preparation.ts` no longer defines a second type; it names the same unavailable value it always hashed, so no fingerprint moved.

The SDK asserts rather than believes. A record must name a path that is its own realpath, a non-symlink regular file, root-owned, with no write bit for anyone, whose bytes hash to the claimed `closureDigest`; the `(dev, ino, size, mtime, mode, uid, gid)` tuple measured at that moment is recorded onto the identity by the SDK and is the one field a host cannot supply. `TaskRunner` resolves one identity per projected server per offer, beside the launch binding, and both production spawn points consume that one value — the admission probe through `McpToolsProbeOptions.implementation`, and the pi extension's pool through `RuntimeOperationStartInput.mcpToolImplementations` and the task-scoped MCP config. `McpStdioClient.connect()` is the shared gate; a reverification failure is an `McpAuthorityError` and the offer declines permanently.

Deviations from the dispatch design, both deliberate: (a) `reverify` is NOT a method on `ToolImplementationAuthority` — the SDK is the one asserting, and the pi extension re-measures in a separate process that holds no resolver handle, so a resolver-owned reverify would have been a second authority over its own record; (b) the attested shape carries an SDK-measured `installStat` the design did not list, because "the lstat tuple unchanged since resolve" is otherwise uncheckable across two processes, and the ownership and write-bit absolutes are asserted once at resolve rather than again at spawn — `uid`, `gid` and `mode` are inside that tuple, so invariance subsumes them.

**Blocked, and where.** Rows G3-wire, G3-service and G3-receipt are NOT closed. The design has the preparation service, for each `toolExecutorClaims` entry, look up "the daemon's own current observation for `{toolsetId, serverName}`", cross-check the caller's fingerprint against it, and call `authority.resolve({toolsetId, serverName, command, args, launch})`. No such lookup exists and none can be added inside this slice:

- The daemon stores no `tools/list` observation anywhere. `McpToolsetRegistry` holds `command`/`args` and `definitionRevisions`, and its `observations` map holds `McpToolsetObservation` — a lifecycle record (`state`, `observedAt`, `version`, `reasonCode`), not tool descriptors. The only producer of an `McpToolsetServerObservation` is a local variable inside `TaskRunner.handleOffer`, handed straight to the adapter and never persisted.
- Producing one inside `prepare()` means spawning MCP servers. `probeMcpServer` requires the exact base environment the RUNTIME child of a task receives, and a preparation call has no task, no workspace and no runtime env. The service holds no reference to `TaskRunner` by construction, and its documented step order and purity contract exclude session/MCP startup outright.

So `authority.resolve` cannot even be called from the preparation path today: the service has no `command`, no `args`, no launch attestation and no observation for a `{toolsetId, serverName}` locator. Closing these rows requires an Owner decision the dispatch does not make — either a daemon-owned observation record that admission writes and preparation reads (new durable authority, new retention question), or a host-supplied observation source on `InputPreparationDaemonConfig` (which puts the cross-check back in the hands of the party it exists to check). The wire break (`toolExecutors` → `toolExecutorClaims`) is deliberately NOT landed ahead of that decision: a wire shape whose claims the service cannot resolve would make preparation fail closed on every request, which is not a coherent intermediate state.

Two design references were checked and do not exist in this repo: `InputPreparationArtifactSummaryV1.executorIdentity` (no occurrence of `executorIdentity` anywhere) and the G4 `agent.input.preparation` envelope (no occurrence at all), so the design's expectation that the G4 payload carries no executors holds trivially. There is no `input-preparation-remote.ts` and no input-preparation schema in `packages/protocol`; `bin/control-client.ts` passes `InputPreparationRequestV1` through unchanged and needs no edit of its own.

### 20. G4-remote channel shape — implemented, 2026-09-15
...
### 21. G3-exec-id phase 2 — one assembly entry, and what stays open — 2026-09-15

§19 recorded rows G3-wire / G3-service / G3-receipt as blocked because the preparation path had no observation to cross-check a caller's fingerprint against, and closing that needed an Owner decision. Two decisions were taken and are implemented here. First, `permissionMode` is DECLARED by the requester and admitted by the device: a preparation counts one concrete manifest, and a manifest is the policy-filtered set for exactly one mode, so a preparation whose mode is unstated is a count of a manifest nobody named. Second, the preparation path resolves its own launch binding with the same functions the task runner uses, and an unprovable boundary refuses rather than spawning.

**The entry.** `packages/client/src/daemon/prepared-tool-surface.ts` is the one place a preparation's tool surface is assembled. It splits by whether anything is SPAWNED. `resolvePreparedToolBinding` takes one immutable registry snapshot, resolves the trusted launch directory through `resolveTrustedLaunchCwd`, and resolves one `ToolImplementationIdentityV1` per projected server through the phase-1 authority — configuration and filesystem reads, no child. `assemblePreparedToolSurface` admits the declared mode, then probes each server through that binding WITH its identity (so `mcp/client.ts`'s connect gate re-measures an attested one before the child starts), applies `filterMcpObservationForPolicy` once, and projects both the model-visible schemas and the executor fingerprints from that one filtered observation. Refusals are values, not exceptions, so an unprovable launch boundary cannot be widened into a spawn by a `catch`.

Both preparation paths reach it through `InputPreparationService.prepare` — the local `input_preparation.prepare` control call and the remote `agent.input.preparation` envelope — and `create-daemon.ts`'s `observeRequiredToolsets` is deleted, not kept beside it. That former path probed with a label, a timeout and an environment and nothing else: no launch binding, no implementation claim. A fingerprint it froze and a fingerprint admission froze could disagree for reasons neither side recorded.

**What a caller may state.** `requiredToolsets` and `permissionMode`, plus authorized prompt text and user history. Not `toolExecutors`, not `snapshot.tools`, and not `snapshot.prompt.selectedTools` — the native contract requires that last list to equal the manifest exactly, so stating it is stating the manifest through the prompt. All three are refused as `unsupported_input` naming the key. `INPUT_PREPARATION_VERSION` is `2`; a version-1 durable record is refused on replay rather than read.

**Admission is not parsing.** The declared mode goes through `computeEffectivePolicy` (`daemon/policy.ts`) against `DaemonConfig.permissionDefaults` — the same merge `TaskRunner.handleOffer` applies to a task offer's `policy` — before a directory is probed for writability and long before a server starts. A mode above the ceiling answers `permission_mode_denied`, never a narrower mode. The `readOnlyTools` cross-check against the live observation is the same `filterMcpObservationForPolicy` the ordinary extension registers through, and it refuses an unclassified toolset under a narrowing mode rather than resolving it to an empty one.

**Replay and drift.** Assembly runs AFTER the durable reserve, so a re-delivery of a recorded `requestId` answers from the record without starting anything. Drift is checked on the half of the evidence that costs no spawn — the launch attestation, the toolset definition revisions, the configured argv and the implementation identities, digested as `toolBindingDigest` on the artifact summary. A repeat whose recorded digest no longer matches is refused with `observation_drift` instead of silently re-derived. The artifact also records `observationDigest` and the per-tool `toolImplementationKinds`, and `executor_identity_unproven` is now emitted from those kinds rather than asserted.

**Still open, and why it is not closed here.** The launch-time seal/pin comparison is NOT implemented. There is no consume point to put it at: nothing outside `input-preparation-store.ts` and its own test ever reads a prepared artifact back, `InputPreparationPinV1` has no writer, and `task.offer` / `task.offer_with_toolsets` carry no preparation reference for a runner to compare against. Adding one means minting the G3b pin and widening the offer payload, which is a separate slice. Until it exists, a preparation's recorded evidence is comparable but never compared, and no receipt can be ready regardless: `coverage: unknown`, a fixture counter authority and an unproven executor identity each keep it not-ready on their own.

**Declared limit.** PARTIAL — the prepared NATIVE tool set is not connected to preparation. Pi's own tools are selected by a runtime policy a task-free path never resolves, so the entry passes `nativeTools: []` and a preparation counts the MCP half only. The final Main set (Q1 = policy-filtered native + MCP) stays the runtime's decision, and nothing here declares an empty MCP set legal or illegal. A test pins the state, so removing the limit must change one.
