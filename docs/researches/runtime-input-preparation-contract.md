...
Current checkpoint: G2 native Session/RPC prepared consume PASS on the artifact-bound local evidence in §17 (`.artifacts/c07-native-input/g2-bridge-r3/`), building on the §16 G1 package declaration closure. The native bridge now admits a prepared first request through the real Session/RPC lifecycle with body equality, pre-transport drift rejection and a held reservation, verified against newly repacked local archives. Native package distribution is resolved per §18: the SDK pins the published fork `@byok-sdk/pi-coding-agent@0.85.1001` through an exact npm alias, so the §17 seam is reachable from an ordinary install. What remains unresolved is G3 trusted Execution/artifact binding (task-runner, pi-adapter, protocol), the B-P2 local primitive (frozen, §10), and G4 accounting/Host CAS/S0 activation; coverage remains `unknown`. Local package acceptance does not make first-batch token admission production-ready.
...
The §17 native closure is now a registry artifact and the SDK pins it. `@byok-sdk/pi-ai`, `@byok-sdk/pi-agent-core` and `@byok-sdk/pi-coding-agent` are published at `0.85.1001`; inside them the `@earendil-works/pi-ai|pi-agent-core` edges are `npm:@byok-sdk/<same>@0.85.1001` aliases, while chord/tui/telemetry stay exact upstream `0.85.1`. `packages/client/package.json` declares `"@earendil-works/pi-coding-agent": "npm:@byok-sdk/pi-coding-agent@0.85.1001"`.
...
**Version scheme.** `<upstream base>.<fork build>`: upstream `0.85.1` plus fork build `1` gives `0.85.1001`, which stays an exact stable SemVer patch above every upstream `0.85.x` release, so an accidental upstream resolution can never satisfy the pin. The fork manifest records `byokFork: {upstreamBase: "0.85.1", upstreamCommit: "d981de1229ef899957bbe968bc8dcda02a21f477", forkBuild: 1}`, and the release gates assert that commit on the installed artifact.
