...
Current checkpoint: G1 package declaration closure PASS on the new exact local artifacts in §16, including MCP, PlatformPath and JSON declaration repairs. External coding-agent and standalone pi-ai strict declarations, SDK/CLI and frozen consume checks pass. B-P2/native Session-RPC designs remain frozen (§10/§11); G2 bridge implementation, G3 trusted Execution/runtime/tool binding and G4 accounting/Host CAS/S0 activation remain unresolved. Package acceptance does not make first-batch token admission production-ready.
