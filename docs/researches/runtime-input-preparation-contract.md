...
Current checkpoint: G2 native Session/RPC prepared consume PASS on the artifact-bound local evidence in §17 (`.artifacts/c07-native-input/g2-bridge-r3/`), building on the §16 G1 package declaration closure. The native bridge now admits a prepared first request through the real Session/RPC lifecycle with body equality, pre-transport drift rejection and a held reservation, verified against newly repacked local archives. G3 trusted Execution/runtime/tool binding and G4 accounting/Host CAS/S0 activation remain unresolved, coverage remains `unknown`, and native package distribution is unresolved — the native checkout is a fork of upstream `earendil-works/pi` while the SDK depends on registry 0.85.1, so no SDK consumer can import this surface yet. B-P2 remains frozen (§10). Local package acceptance does not make first-batch token admission production-ready.
