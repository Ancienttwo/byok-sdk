...
for label,root in [('base',Path('/Users/kito/Projects/byok-sdk-wt-doctor-baseline')),('candidate',candidate)]:
    ...
    for kind in ['sync-delay-v2']:
        ...
        env['DOCTOR_PROFILE_OUTPUT']=str(out/(label+'-sync-v2-observations.jsonl'))
        started=time.time()
        try:
        ...
        results.append(row);(out/'control-v2-results.json').write_text(json.dumps(results,indent=2)+'\n')
