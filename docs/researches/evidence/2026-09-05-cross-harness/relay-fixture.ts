...
import { parseTeamNotificationSnapshotParams, parseTeamMessagePostParams, parseTeamMessageReadParams, parseTeamMessageAckParams, ControlError } from '../../../../packages/client/src/daemon/control-protocol';
...
if (mode === 'helper') {
  ...
  await workspace.createWorkspace({ workspaceId: 'probe-room', members: ['alice', 'bob'], limits: { maxMembers: 2, maxMessages: 64, maxBytes: 65536 } });
  ...
  const handle = await startControlServer({ storeDir, productId, methods: { stream: {}, unary: {
    'team_notifications.snapshot': async params => {
      const p = parseTeamNotificationSnapshotParams(params);
      if (!p) throw new ControlError('bad_request', 'invalid snapshot');
      return workspace.notificationSnapshot({ lease: decodeTeamMemberContext(p.context), ...(p.afterSeq === undefined ? {} : { afterSeq: p.afterSeq }) });
    },
