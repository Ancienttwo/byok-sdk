...
- list/get 之间若 rev/hash/size/label/time 任一变化即 fail-closed并要求 caller 重走 manifest decision。object download 只允许 host 显式配置的 credential-free HTTP(S) origin，禁止自动 redirect；再用声明 size 做 bounded stream read，并按 daemon 声明摘要 rehash。同 size 字节替换或恶意 grant URL 都不能进入 filter/内网请求面。
- client 提供 snapshot/terminal proof write，inline hash由 client从 exact bytes计算；object body只接受已经完成 reservation/finalize 的 canonical hash/size。write requestId 由 caller持有，重试不重新生成 id；成功 response 的 primary/ordered snapshots 必须逐项匹配请求 selector、next revision、hash 与 size，否则以 typed mismatch fail-closed。
