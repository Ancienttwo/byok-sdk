...
- stale owner/reclaim 的 check-remove-publish 另受 canonical-realpath-derived kernel-exclusive loopback mutex 串行，symlink/junction alias 必须落到同一 mutex，消除两个 reclaimer 互删 marker/owner 的 TOCTOU；mutex port hash collision 只会 fail-closed 拒绝无关 store 的 concurrent mutation，不会放进第二 writer。owner record 仍使用独立 random liveness listener 裁定 PID reuse。device 与 audit tail 都在任何 byte 读取前后完成 pathname/handle identity binding；POSIX evidence/manifest link 后先 fsync quarantine directory，才 unlink source 并 fsync source parent；Windows 保留 file fsync + ordered link/unlink，因为 Node/libuv 没有 directory flush surface；
