...
- malformed/corrupt health state 在 ordinary status/doctor 下保持 byte-identical；read/open/permission/race failure 单独投影为 `unavailable`，不得冒充 confirmed corruption；daemon 从 start 到完整 writer teardown 持有 cross-process store mutation lease，failed start 或 stop barrier 未闭合时继续 fail-closed 持有；只有 operator 明确给出 `doctor --fix --yes`、成功取得同一 lease，且由 no-follow/non-blocking handle 重新确认 JSON/shape corruption 时才可搬进 quarantine；fix 对同一个已打开 inode 做 atomic rename、identity 比对与 SHA-256，再写 source path、reason、size manifest，不创建伪造 healthy replacement；
