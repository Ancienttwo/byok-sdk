...
| Decide whether the change affects boundaries/entrypoints/dependency rules/runtime paths/verification | done — K0 events: yes, the new `@byok-sdk/keys` credential plane boundary is recorded in canonical §7. Publish-metadata events: no, verified field by field in the 1405 snapshot |
